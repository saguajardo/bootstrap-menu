...
trait HasRoleAndPermission
{
    ...
    {
        return $this->belongsToMany(config('bootstrap-menu.models.role'), config('bootstrap-menu.relations.role_user'))->withTimestamps();
    }
     ...
     *
     * @param int|\Saguajardo\BootstrapMenu\Models\Role $role
     * @return null|bool
     */
    ...
    {
        $permissionModel = app(config('bootstrap-menu.models.permission'));

        if (!$permissionModel instanceof Model) {
            throw new InvalidArgumentException('[bootstrap-menu.models.permission] must be an instance of \Illuminate\Database\Eloquent\Model');
        }

        return $permissionModel::select([config('bootstrap-menu.tables.permissions') . '.*', config('bootstrap-menu.relations.permission_role') . '.created_at as pivot_created_at', config('bootstrap-menu.relations.permission_role') . '.updated_at as pivot_updated_at'])
                ->join(config('bootstrap-menu.relations.permission_role'), config('bootstrap-menu.relations.permission_role') . '.permission_id', '=', config('bootstrap-menu.tables.permissions') . '.id')->join(config('bootstrap-menu.tables.roles'), config('bootstrap-menu.tables.roles') . '.id', '=', config('bootstrap-menu.relations.permission_role') . '.role_id')
                ->whereIn(config('bootstrap-menu.tables.roles') . '.id', $this->getRoles()->lists('id')->toArray()) ->orWhere(config('bootstrap-menu.tables.roles') . '.level', '<', $this->level());
                //->groupBy(['d_biods_v.permissions.id', 'pivot_created_at', 'pivot_updated_at']);
    }
    ...
    {
        return $this->belongsToMany(config('bootstrap-menu.models.permission'), config('bootstrap-menu.relations.permission_user'))->withTimestamps();
    }
    ...
    {
        return (bool) config('bootstrap-menu.pretend.enabled');
    }
    ...
    {
        return (bool) config('bootstrap-menu.pretend.options.' . $option);
    }
    ...
    public function __call($method, $parameters)
    {
        if (starts_with($method, 'is')) {
            return $this->is(snake_case(substr($method, 2), config('bootstrap-menu.separator')));
        } elseif (starts_with($method, 'can')) {
            return $this->can(snake_case(substr($method, 3), config('bootstrap-menu.separator')));
        } elseif (starts_with($method, 'allowed')) {
            return $this->allowed(snake_case(substr($method, 7), config('bootstrap-menu.separator')), $parameters[0], (isset($parameters[1])) ? $parameters[1] : true, (isset($parameters[2])) ? $parameters[2] : 'user_id');
        }
